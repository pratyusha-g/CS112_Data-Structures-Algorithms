...
public class WordPalindromeTest {
	...
	public static boolean isWordPalindrome(String s) {
		...
		//added the remove \n cos I want to pass the autograder but it's not working...
		//makes it alot slower though...
		s = s.replaceAll("\\\\n", " ");
		s = s.replaceAll("[^a-zA-Z0-9 ]", "");
		
		//now comvert string into array of strings
		 ...
		 *need to check each word front and back to see if the same, and same order
		 */
		
		//now check palindrome using mirror image analysis!!
		//
		int n = array_s.length;
		int count = 0;
		//(n/2) used here cos first half read forwards and second half read backwards should be same
		for (int i=0; i<(n/2); ++i) {
			//compare words in the array
			if (array_s[i].equals(array_s[n-i-1])) {
				count += 1;
			}
	 ...
	 // Declare a scanner object for user input
	    ...
	    while (userInput.hasNextLine()) {
	        ...
	        if (line.equals("quit")) {
	          ...
	          boolean result = isWordPalindrome(line);
